...
Storage audited on 2026-09-22; queue/allocation values below were last verified on 2026-08-28 and must be rechecked before a production submission.
...
- Main links: `.venv -> /scratch/ql44/we2614/mts-spi-study/environments/mts-spi-v3-631de27`, `data -> /scratch/ql44/we2614/mts-spi-study`, `logs -> /scratch/ql44/we2614/mts-spi-study/operations/logs/pbs`. Read the [storage contract](docs/operations/gadi-storage-layout.md) before adding or moving cluster artifacts. Use `documentation/storage-index.csv` in the project root to find physical owners; `python scripts/gadi_storage_path.py OLD_PATH` resolves historical paths. The dataset-farm submitter checks the layout and output placement before submission.
- Allocation: Scratch 1 TiB / 202k inodes; gdata 100 GiB / 70k inodes. After the 2026-09-22–23 cleanup, live use was Scratch 171.60 GiB/116,434 inodes (57.6%) and gdata 36.45 GiB/64,531 inodes (92.2%). Gdata inode headroom remains limited; check both byte and inode quotas before another loose-file farm. Keep durable code in Git; Scratch remains temporary working storage. See the [storage audit](docs/operations/gadi-storage-audit-260922.md) and [run inventory](docs/operations/gadi-storage-inventory-260922.csv).
- Canonical project roots are `/g/data/ql44/we2614/mts-spi-study/` and `/scratch/ql44/we2614/mts-spi-study/`: `proof/`, `order-parameter-inference/<system>/`, `zenodo/7118947/`, `representation/`, `archives/<workstream>/`, `operations/{sources,logs,maintenance}/`, `environments/`, optional Scratch `dev/`, `legacy-non-p90/` and `documentation/`. Cross-M/T data/features/analysis belong to `representation/cross-mt/`; the shared multi-system bank stays in `proof/`. Mirror categories, not contents. Consult the index before assuming a run exists on a particular store. No individual workstreams, `mts-spi-data*`, `mts-spi-archives`, `venvs` or compatibility aliases at store roots.
...
- The old 820-record short-burn `cml_param_sweep_260508` data bank is authorised for deletion; its compact derived feature bank and the distinct long-burn quadratic-CML/CML2D banks are retained. TASEP, Ising and superseded execution smokes were deleted. Non-`benchmarked_p90` SPI outputs remain flagged, not automatically deleted; physics-only validation is exempt.
...
- `nci_account` reports the spendable quarterly allocation. Live 2026.q3 balance: 143 KSU granted, 31.12 KSU used, 111.88 KSU available.
