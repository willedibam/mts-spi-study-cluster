    ...
    *,
    ...
    for cluster_id in sorted(set(labels)):
        ...
        for _, row in subset.iterrows():
            ...
            heatmap_src = dataset_abs / "mts_heatmap.png"
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    def _pattern_list(value: str | None) -> list[str]:
        if not value:
            return []
        return [token.strip() for token in str(value).split(",") if token.strip()]

    include_patterns = _pattern_list(args.dataset_filter)
    exclude_patterns = _pattern_list(args.dataset_exclude)

    def _matches(patterns: list[str], candidates: list[str]) -> bool:
        return any(
    ...
    for rel, norm, disp, base in zip(
        ...
        include_ok = True
        if include_patterns:
            include_ok = _matches(include_patterns, candidates)
        exclude_hit = _matches(exclude_patterns, candidates) if exclude_patterns else False
        mask_entries.append(include_ok and not exclude_hit)

    mask = np.array(mask_entries, dtype=bool)
    if not mask.any():
        raise ValueError("Dataset filters removed every dataset.")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated glob patterns to INCLUDE (e.g. 'data/dev/CML/*,Kuramoto').",
    )
    parser.add_argument(
        "--dataset-exclude",
        type=str,
        default=None,
        help="Comma-separated glob patterns to EXCLUDE (matched against dataset paths/classes).",
    )
    parser.add_argument(
        "--light",
        dest="theme",
