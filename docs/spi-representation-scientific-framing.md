...
## Post-InceptionTime narrative and next decision

The user explicitly rejects universal neural superiority as the intended claim. Lead with a positive contribution: cross-statistic correspondence provides a transferable representation of interaction organization, making the selected functional property accessible to simple predictors with limited source labels. Construction, correspondence interventions, source-frozen dynamics/mechanism transfer, comparison with statistical and learned alternatives, and selective failure form the argument. NeuroTycho adds competitive external transfer, not the primary mechanistic proof. Negative results delimit the prior; they do not erase the positive result. Strong framing should emphasize the demonstrated capability rather than increase the breadth of the claim.

Recommended next steps are synthesis plus one bounded baseline decision: MiniRocket with its conventional ridge classifier on the existing full-size synthetic cohorts/targets would test whether the observed benefit is shared by a generic fixed feature transform. This is a materially different comparison from InceptionTime; no new generator or larger repeat is needed. It is a recommendation, not a newly frozen protocol or launched run.

For a new real-data study, audit HCP working-memory MEG before committing to computation. The [HCP manual](https://www.humanconnectome.org/storage/app/media/documentation/s900/HCP_S900_Release_Reference_Manual.pdf) documents 0-back/2-back load crossed with face/tool stimuli. A candidate question is whether a source-trained representation transfers the load contrast to unseen participants and the other stimulus category, while retaining information beyond spectra and separate coupling summaries. The existing [Siems and Siegel study](https://pmc.ncbi.nlm.nih.gov/articles/PMC7068703/) establishes nonredundant phase/amplitude coupling patterns in resting HCP MEG and directly compares those patterns; it motivates the physical subject matter but does not establish our task effect or methodological novelty. Require usable continuous/block time series, sufficient independent participants and within-condition duration, repeatability, artifact/leakage handling and bounded preprocessing cost before accepting this experiment. The usable task cohort, access and reconstruction feasibility remain unaudited. Do not substitute group-averaged connectivity for per-record raw time series or label repeated blocks as independent people.

Do not choose the attractive dream-report contrast in OpenNeuro ds005620 as the next confirmatory task: the [primary paper](https://www.nature.com/articles/s41598-025-12695-z) reports 24 experience reports, only five no-experience reports and 23 uninformative attempts among 52 awakenings. These are not independent participants. Its scientific importance does not compensate for the limited negative-class support. No new data downloaded, fits launched or monitor restarted for this strategy discussion.

### HCP access and multimodal scope audit, 2026-09-12

The live `https://db.humanconnectome.org/` now redirects to [BALSA](https://balsa.wustl.edu/), superseding older ConnectomeDB/Aspera instructions. [HCP–Young Adult 2025](https://balsa.wustl.edu/project?project=HCP_YA) lists 95 MEG participants and requires login plus acceptance of WU-Minn HCP Open Access terms; the page also exposes an AWS S3 access route after access is configured. User is unsure of existing access and is willing to register. No credentials requested or data downloaded. No HCP-specific files found in the inspected local data/results/config paths. Current usable working-memory cohort and package access remain unverified. HCP contains twins/siblings; obtain an appropriate family grouping or otherwise explicitly limit the independence claim before a held-out-subject confirmation. The [MEG FAQ](https://wiki.humanconnectome.org/docs/MEG%20Data%20FAQ.html) notes that several released task source products average responses across trials; these are not substitutes for continuous or single-trial time series. Source models already provide anatomical alignment across people, so dimensional comparability is not uniquely enabled by SPI–SPI.

The user's multimodal idea is technically possible for synchronized signals but conventional polysomnography already combines EEG, EOG and EMG; [AASM scoring examples](https://isr.aasm.org/helpv4/generalinstructions.html) make this explicit. Stacking different modalities removes neither sampling/filtering differences nor measurement mixing, and untyped z discards which edge is EEG–EEG versus EEG–EOG. Modality counts change the edge mixture, so identity blindness can be a disadvantage. Consider modality-specific SPI–SPI blocks only if that becomes the explicit scientific question; do not silently change the current method or add a sleep programme. Human seizure corpora such as [CHB-MIT](https://physionet.org/content/chbmit/1.0.0/) have multichannel recordings and onset labels, but clinically useful onset detection additionally requires continuous-stream false-alarm and latency evaluation. Comparable dimensions alone do not resolve spatial coverage or species/measurement differences. Public marmoset recordings exist, but a suitable cross-species transition-labelled cohort has not been established here.

The dream-report dataset is not scientifically worthless: a fixed, small exploratory pilot could assess feasibility and generate effect-size hypotheses. Five no-experience reports, repeated within participants, remain insufficient support for the proposed confirmatory classifier comparison regardless of available compute. No exploratory dream run has been launched; HCP feasibility and the already-frozen synthetic comparator take priority.
