...
Authoritative handoff as of 2026-08-24. Repository: `~/Code/mts-spi-study-cluster`, branch `refactor-lagged-warping`; local and Gadi HEAD `fbbb9036de4ae72f9f4d50e08692e27982a13cad`. Facts below were checked against code, artifacts or PBS state; proposals are labelled.
...
2. **Goal 2 — quantitative representation evaluation, only after Goal 1.** Compare frozen `z_sym`, `z_dir` and development-fitted augmented variants on held-out tasks. Kuramoto and the existing CML panel are complete and do not favour augmentation. Exact proof and CML-order archives were absent, so current-pyspi regeneration is a sensitivity, not numerical reconstruction of the old archives.

PBS snapshot at handoff: proof main/bridge farms `177205470` (720 rows) and `177205481` (180 rows), plus CML-order farm `177206563` (820 rows), are running. They target gdata-backed `data/embeddings/multi_p90_260701` and `data/embeddings/cml_param_sweep_260508`; no feature reconstruction or analysis has yet been submitted for these new corpora. Successful generator smokes were `177204206` and `177204219`. No jobs were launched or changed while writing this handoff.
