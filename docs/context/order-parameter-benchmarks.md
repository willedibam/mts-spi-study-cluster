...
Stuart--Landau full-p90 development passed its target-blind geometry gate: all 289 SPIs entered; ordinary hygiene retained 24,466 pairs spanning 234 SPIs; PC1 explained `.661`, was the strongest of the first ten PCs on the held diagnostic, and had loading cosine `.977-.987` across all six `M x T>=500` source fits. On instances 4--7, per-cell mean `rho(q,Q)=.85-.99` and the pooled gamma-mean curve has `.976`; raw pooled rows have `.756`. `T=100` is useful as a cell-level stress test but its paired q-rank agreement with `T=1000` is only `.628/.740/.944` at full `M=8/16/32`; `T=500` improves to `.777/.909/.957`. Do not add `T=2000`. Within-gamma recovery is weak, and analytic phase coherence is a stronger baseline (`rho=.932`, within-gamma `.779`); the claim is changing order-coordinate recovery, not superiority or microscopic fluctuation prediction. Independent new-seed, interleaved-gamma confirmation is active.
