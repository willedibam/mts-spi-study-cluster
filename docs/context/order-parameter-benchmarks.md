...
- Independent channel shifts attenuated overall absolute association from `.936` to `.589` (Gaussian) and `.948` to `.438` (logistic); the clustered difference intervals excluded zero. This is consistent with cross-channel temporal alignment contributing to the representation. Within-coupling attenuation was conclusive only for logistic. Shifting also induced maximum selected-feature missingness `.136`; estimator failure, frozen imputation and retained shifted association prevent clean causal attribution.
