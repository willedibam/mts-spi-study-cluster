...
- The repository exactly matches the original synchronous `mu=3` map. Existing `L={32,64,128}` scouts reproduce the sharp rise near refined `g_c=.20534(2)`, susceptibility peak and start-state agreement. Near-critical 40k-step truth blocks are not stationary enough; extend to at least 200k updates before p90.
...
1. **Headline candidates:** Miller--Huse, conditional on resolving near-critical truth uncertainty; and Stuart--Landau, whose physics gate passed. The latter's `K=.8`, `gamma=.6,.8,1.0,1.2` path is exactly Fig. 1 of Matthews--Strogatz, not an arbitrary slice: it traverses locking, large oscillations, irregular order-parameter motion and incoherence. It keeps the published `R=|mean z_j|` while avoiding amplitude-death constant channels.
