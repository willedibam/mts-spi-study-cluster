...
Current Gadi launcher/path changes were deployed from `codex/gadi-storage-layout` (through commit `8fc2bc1`, based on the previous Gadi main-checkout commit `18ef949`). Matching path edits and the migration branch history were integrated into the local `refactor-lagged-warping` branch during the September 23 local review, preserving newer local code and user notebook changes. No cluster checkout was updated during that local review. Historical generated YAML/metadata remain provenance records. Config/source hashes are part of completion identity, so a path edit alone must not trigger overwriting a retained scientific result; replay needs explicit translated paths and the correct original identity.
...
## Retained July notebook dependency, September 23

The user retained `notebooks/embeddings/proof_p90_260712.ipynb`. Its CML panel loads local `features/data-embeddings-cml-embedding_pearson.npz` (540 rows, 297 SPIs), whose stored source paths are the legacy `cml-embedding` bank. Its multi panel also uses a 297-SPI bank despite the p90 filename. The legacy CML bank remains non-current-catalogue, but deletion is conditional on no longer needing to recompute this retained notebook’s features. Cached notebook plots do not directly require the Gadi raw bank. This qualifies the earlier blanket deletion recommendation.
