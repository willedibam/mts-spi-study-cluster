...
def analyze(corpus, mpi_root, output, frozen=None):
    ...
    return analyze_assembled(rows, z, order, sources, corpus, output, frozen)


def analyze_assembled(rows, z, order, sources, corpus, output, frozen=None, status=None):
    """Analyze assembled features; cached callers must verify their provenance first."""
    if output.exists():
        raise FileExistsError(output)
    ...
    summary = dict(status=status or ('frozen confirmation' if frozen else 'exploratory held-seed diagnostic'),
