...
The three mixtures contain (linear, sigmoid, quadratic) channel counts (32,0,0), (16,16,0), (12,12,8). Within each mixture, the β variants share the same mother and noise draws; mixtures use independent seeds. The all-linear baseline is computed once and reused in the three panels. These are illustrative constructions, not universal claims about all linear or nonlinear systems.'''),
            ...
            md('The qualitative pattern persists across β. In the linear/sigmoid mixture, mean r–ρ agreement is 0.188, 0.215 and 0.270 at π, 5 and 2π: the old monotonic-decrease claim does not hold for these settings. Raw MI is a nonlinear function of correlation even for Gaussian pairs, so the all-linear r–MI agreement need not equal one.'),
            md('The SPI–SPI planes below use the predetermined instance 0 at β=5. Grey points are same-family channel pairs; coloured points are mixed-family pairs (L=linear, M=sigmoid, Q=quadratic). The annotation uses all pairs. Axes are independently scaled to resolve each cloud.'),
            ...
            md('Across all 350 datasets, temporal exclusion changes r–MI and ρ–MI agreement by median absolute amounts 0.0013 and 0.0007; the largest change is approximately 0.010. These changes do not alter the qualitative case-study pattern.')]
    else:
        cells += [md(r'''Each channel is a noisy copy of an AR(1) mother ($a=0.5$), with a fixed lag and independent rebound warping. We retain $M=20$, $T=1000$, 50 instances/condition, lag range 0–5, fixed excursion size 3, noise base 0.1 and uniform multiplier range $[1/2.718,2.718]$. The first control has neither lag nor warping; the second has lag only.
...
All distances use the same squared-error cost and division by $\sqrt{T}$. The shifted distance minimises over shifts −10 through 10, including boundary costs; unrestricted DTW searches a larger set of admissible paths. Therefore DTW ≤ shifted ≤ Euclidean is a structural property, not evidence that DTW recovered the physical warp. The divisor is the record length, **not warping-path length**.'''),
            md('Lines show mean SPI–SPI Pearson agreement; bars are pointwise 95% Student-t intervals over the 50 independent MTS instances. The first control is separated because it has a different lag range.'),
            code("fig = plot_dtw_sweep(frame, OUT)\nplt.show()"),
            md('Lag alone separates Euclidean distance from both alignment-aware distances, while DTW–shifted agreement remains 0.995 on average. Warping reduces the latter to 0.51–0.59; it does not produce a strictly monotonic decline at every step. No distance-hierarchy violations occur among the 66,500 unique channel pairs across the 350 datasets.'),
            md('Predetermined instance 0 illustrates no lag/no warp, lag only, and heavy warping. Colour shows the absolute generating lag difference. Dashed lines mark equal distances. Each panel uses matching x/y scales, with limits varying between panels.'),
