...
The equations follow from stationarity and innovations independent of the past: $L=AC$ and $C=ACA^\top+\Sigma_\epsilon$. Thus A predicts the next state from the previous one, and $\Sigma_\epsilon$ supplies the remaining covariance needed to maintain C. The innovations may be correlated across channels; C and L are not direct causal-coupling matrices.
...
The offsets set the means and the positive amplitudes set the spread across edges. These exact decimals are illustrative choices, not fitted physical parameters or uniquely justified constants; their checked role is to provide heterogeneous profiles in a valid stable VAR. The lagged offset .01 is not essential to the correspondence argument. No parameter-robustness study is claimed.

Here $z(C,L)=\mathrm{corr}_{i\ne j}(C_{ij},L_{ij})$: a scatterplot would put one channel pair at $(C_{ij},L_{ij})$, with an upward line for h=+1 and a downward line for h=−1. The symbol h is **binary**, not a continuous correlation parameter. Extending this same formula to nonzero real h gives $z=\mathrm{sign}(h)$; at h=0 the constant lagged profile makes z undefined.
...
("md", """**Reading the result:** SPI–SPI improves hard retrieval over the seven-summary baseline: all-class mAP .818 versus .714; CML mAP .940 versus .766. The all-class classification gain (.983 versus .925) is much more concentrated: about 98% of its **net** gain comes from `var-phi-0.2_cpl-0.4` and `var-phi-0.2_cpl-0.8`. These classes contribute 144 of the 147 net additional correct predictions. On the remaining 12 classes, the unchanged classifiers score .9861 (z) versus .9847 (distributions); this is a descriptive evaluation slice, not a refitted 12-class experiment. Several classes are already at ceiling. Thus this is not broad evidence for detecting nonlinear mechanisms. Brownian-defect is worse under z. Within CML, classification is near ceiling and its z–distribution difference is unresolved, whereas the retrieval difference is substantial. Equal-variance fusion improves on the corresponding marginal baseline but does not uniformly improve on z.
...
All six system families from the [lean benchmark notebook](../inference/order-parameter-benchmarks-lean.ipynb) are shown at their headline observation sizes. Stuart–Landau uses only the local sweep around the locking boundary; the second Kaneko diagnostic is retained as an additional panel. Original model fitting and evaluation divisions are preserved:
...
- **Stuart–Landau:** globally coupled complex oscillators; Q is mean collective amplitude. The original 240 full-observation development rows (all three M, T≥500, instances 0–3) fit each baseline once. Evaluate the 152 local-sweep confirmation recordings at M=32, T=1000 with that fitted model; the original development fit is retained. The original full-arm qualification is retained; the failed partial-arm global gate is not erased.
...
("code", "fig = plots.inference(('Kuramoto', 'StuartLandauFine', 'MillerHuse'), filename='inference-families-1')\nplt.show()"),
("code", "fig = plots.inference(('KanekoBand', 'CML2D', 'Rossler'), filename='inference-families-2')\nplt.show()"),
("code", "fig = plots.inference(('KanekoEntropy',), filename='inference-additional-targets')\nplt.show()"),
("code", "display(plots.inference_table(compact=True, systems=plots.HEADLINE_SYSTEMS))\ndisplay(plots.inference_paired_table(systems=plots.HEADLINE_SYSTEMS))"),
...
**Added-system results:** On the local Stuart–Landau sweep, distribution-PC1 has somewhat higher association than q; its mean-PC1 comparison is unresolved in the paired intervals. Miller–Huse's small mean/distribution differences are also unresolved. Rössler slightly favours q: its mean-PC1 difference is unresolved, while distribution-PC1 is lower. These are conditional, unadjusted comparisons of the specified pipelines.
...
This notebook renders the compact results produced by `scripts/spi_baseline_exploration.py`; it does not silently recompute p90. Protocol: `configs/analysis/spi-baseline-exploration-260921.yaml`. Input inventory: `data/baseline-comparison_260921/proof-inputs.json`. All proof, Kuramoto and CML MPI archives are checked against their earlier recorded SHA-256 values. Source hashes and original-q replay are retained in the output provenance JSONs.

From the repository root, use the project Python to run `python -m scripts.spi_baseline_exploration toy`, `extract-proof`, `proof`, `inference`, and optionally `zenodo`. Missing cached proof files can be inventoried with `scripts/prepare_spi_baseline_data.py`; the resulting rsync list is for Gadi data-mover retrieval, not simulation. The original notebooks are unchanged. The separate corpus appendix is descriptive and transductive. Static figure files, scores, per-record outcomes and provenance live in `results/baseline-comparison_260921/`."""),
...
Reproduce with `python -m scripts.spi_baseline_exploration zenodo` from the repository root. It reads only the seed1729 MPI bank. Compact arrays, source hashes and descriptive metrics are saved under `results/baseline-comparison_260921/`; the exact formulas and remaining comparisons are in the main notebook."""),
