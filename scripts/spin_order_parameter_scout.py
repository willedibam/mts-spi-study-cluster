#!/usr/bin/env python3
"""Physics-only Miller--Huse scout; no SPI computation."""
...
from src.generators.order_parameter import generate_miller_huse  # noqa: E402
...
def _integrated_autocorrelation_time(values: np.ndarray) -> float:
    """Initial-positive-sequence estimate for a scalar stationary series."""

    centered = np.asarray(values, dtype=np.float64) - np.mean(values)
    n = centered.size
    variance = float(np.dot(centered, centered) / n)
    if n < 4 or variance <= 0.0:
        return 1.0
    size = 1 << (2 * n - 1).bit_length()
    spectrum = np.fft.rfft(centered, n=size)
    autocovariance = np.fft.irfft(spectrum * np.conj(spectrum), n=size)[:n]
...
def run_one(config: dict, job: dict) -> dict:
    ...
    q_abs_tau = _integrated_autocorrelation_time(np.abs(future))
    record = {
        "model": model,
        **job,
        "seed": seed,
        ...
        "q_future_abs_tau_int": q_abs_tau,
        "q_future_abs_effective_samples": float(len(future) / q_abs_tau),
