...
def main(root, phase='source'):
    fig, axes = plt.subplots(4, 2, figsize=(12, 12))
    reports = []
    metadata = [json.loads(p.read_text()) for p in root.glob('201*.json')]
    if len(metadata) != 4:
        raise ValueError('expected four QC archives')
    for row, meta in enumerate(sorted(metadata, key=lambda m: (m['animal'], m['archive']))):
        ...
        for target, color, label in [(0, 'C0', 'Awake, eyes closed'), (1, 'C1', 'Anaesthetized')]:
            ...
            reports.append(dict(animal=meta['animal'], archive=meta['archive'], state=label, accepted=len(records),
        ...
        axes[row, 0].set_title(meta['animal'] + ' ' + meta['archive'][:8] + ': first three bipolar channels')
    ...
    fig.suptitle(f'{phase.capitalize()} QC: fixed midpoint windows, identical preprocessing\nAmplitude units remain those of the public files; no physical-voltage calibration assumed')
    fig.tight_layout(rect=(0, 0, 1, .95))
    fig.savefig(root / f'{phase}-qc.png', dpi=150)
